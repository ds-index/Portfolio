...
const AboutMe = () => {
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-[radial-gradient(circle_at_top_left,_rgba(79,70,229,0.16),_transparent_35%),linear-gradient(135deg,_#020617_0%,_#0f172a_45%,_#111827_100%)] text-slate-100">
      <main className="mx-auto flex max-w-7xl flex-col gap-10 px-4 py-10 sm:px-6 lg:flex-row lg:items-center lg:justify-between lg:px-8 lg:py-16">
        ...
        <div
          ...
          <div className="mb-4 inline-flex items-center rounded-full border border-indigo-400/30 bg-indigo-500/10 px-3 py-1 text-sm font-medium text-indigo-200">
            Backend Engineer • DevOps Enthusiast • Software Architecture
          </div>
          ...
          <div className="mt-6 space-y-4 text-base leading-8 text-slate-300 sm:text-lg">
            <p>
              I specialize in building scalable backend applications and RESTful
              APIs using .NET and modern software architecture principles. My
              focus is on designing maintainable, high-performance systems that
              solve real-world business challenges.
            </p>

            <p>
              Experienced with C#, .NET, ASP.NET Core, Angular, Typescript,
              Entity Framework Core, Dapper, LINQ, Identity, JWT, SQL Server,
              PostgreSQL, MongoDB, Redis, Docker, Kubernetes, Git, and
              Microservices Architecture. I enjoy building reliable software
              while applying Clean Architecture, SOLID principles, Domain-Driven
              Design, CQRS, and System Design best practices.
            </p>

            <p>
              Currently expanding my expertise in Golang, DevOps, Cloud
              Computing, Infrastructure Automation, CI/CD, and cloud-native
              technologies. Passionate about building scalable, resilient
              systems and continuously improving development and deployment
              workflows.
            </p>
          </div>
          <p className="mt-6 text-lg font-semibold text-indigo-300">
            “Building scalable architectures, reliable systems, and cloud-native
            solutions.”
          </p>
        </div>
      </main>
      ...
      <section
        className={`mx-auto max-w-7xl px-4 pb-8 sm:px-6 lg:px-8 ${loaded ? "translate-y-0 opacity-100" : "translate-y-10 opacity-0"}`}>
      ...
      <section
        className={`mx-auto flex max-w-7xl justify-center px-4 pb-10 sm:px-6 lg:px-8 ${loaded ? "translate-y-0 opacity-100" : "translate-y-10 opacity-0"}`}>
        <a
          href={resumeFile}
          download
      ...
      <footer
        className={`border-t border-white/10 px-4 py-6 text-center text-sm text-slate-400 ${loaded ? "translate-y-0 opacity-100" : "translate-y-8 opacity-0"}`}>
