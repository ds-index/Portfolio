      ...
      <div className="mx-auto max-w-6xl">
        <section className="mb-16">
          <div className="mb-8 text-center">
            ...
            <h1 className="text-3xl font-black sm:text-4xl md:text-5xl">Certificates</h1>
          </div>
          ...
          <div className="relative mx-auto flex max-w-4xl flex-col gap-6">
            ...
            {certificates.map((cert, index) => {
              ...
              return (
                <div key={cert.name} className="relative flex items-center">
                  ...
                  <div className={`w-full md:w-[calc(50%-1.25rem)] ${isLeft ? "md:mr-auto md:pr-10 md:text-right" : "md:ml-auto md:pl-10 md:text-left"}`}>
                    <div className="rounded-[1.5rem] border border-white/10 bg-slate-900/70 p-5 shadow-[0_20px_50px_rgba(0,0,0,0.16)] backdrop-blur-xl transition duration-300 hover:-translate-y-1 hover:border-indigo-400/40">
                      <div className={`flex items-center gap-4 ${isLeft ? "flex-row-reverse md:flex-row-reverse" : "flex-row"}`}>
                        <img src={cert.icon} alt={cert.name} className="h-12 w-12 flex-shrink-0" />
                        <div className="min-w-0">
                          <h2 className="text-xl font-bold text-white">{cert.name}</h2>
                          <p className="mt-1 text-sm text-indigo-200">{cert.date}</p>
        ...
        <section>
          <div className="mb-8 text-center">
            ...
            <h1 className="text-3xl font-black sm:text-4xl md:text-5xl">Work Experience</h1>
          </div>
            ...
            {experiences.map((exp, idx) => (
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <h2 className="text-2xl font-bold text-emerald-200">{exp.role}</h2>
                    <p className="mt-1 text-sm text-slate-300">{exp.company}</p>
                    <p className="mt-2 text-sm text-slate-400">{exp.duration}</p>
                  </div>
      ...
      {selectedExp && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
          <div className="relative w-full max-w-lg rounded-[1.75rem] border border-white/10 bg-slate-900 p-8 shadow-[0_20px_80px_rgba(0,0,0,0.45)]">
            ...
            <h2 className="mb-3 text-3xl font-bold text-emerald-200">{selectedExp.role}</h2>
            <p className="mb-2 text-sm text-slate-300">Company: {selectedExp.company}</p>
            <p className="mb-2 text-sm text-slate-300">Duration: {selectedExp.duration}</p>
            <p className="mb-4 text-sm text-slate-300">Technologies: {selectedExp.tech.join(", ")}</p>
            <p className="text-slate-200">{selectedExp.description}</p>
          </div>
        </div>
      )}
